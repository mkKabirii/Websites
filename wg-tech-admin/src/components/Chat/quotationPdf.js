...
const DEFAULT_CONTENT = {
  ...
  support:
    "1 Month free bug-fix support after delivery. This quotation is valid for 15 days from the date of issue. If you need any modifications, feel free to let me know. Looking forward to working with you.",
  notes:
    "Client will provide Namecheap domain & Heroku credentials. All deliverables include responsive design, deployment and testing. Extra features (e.g., admin panel, e-commerce, membership) may incur additional charges. The client will provide the necessary accounts for publishing the app: a Google Play Store account for Android and an Apple App Store account for iOS, if required.",
  milestones: [
...
const drawStyledLogoText = (doc, centerX, baselineY) => {
  const green = [124, 185, 0];
  const leftPart = "WGTECH S";
  const middlePart = "O";
  const rightPart = "L";
  ...
  const totalWidth =
    doc.getTextWidth(leftPart) +
    doc.getTextWidth(middlePart) +
    doc.getTextWidth(rightPart);
  let x = centerX - totalWidth / 2;

  doc.setTextColor(0, 0, 0);
  doc.text(leftPart, x, baselineY);
  x += doc.getTextWidth(leftPart);

  doc.setTextColor(...green);
  doc.text(middlePart, x, baselineY);
  x += doc.getTextWidth(middlePart);

  doc.setTextColor(0, 0, 0);
  doc.text(rightPart, x, baselineY);
};
...
const drawHeader = async (doc, pageWidth, options = {}) => {
  ...
  const titleEndX = logoX - titleLogoGap;
  const titleCenterX = (titleStartX + titleEndX) / 2;

  doc.setTextColor(0, 0, 0);
  doc.setFont("helvetica", "bold");
  doc.setFontSize(20);
  doc.text("WGTECH SOL", titleCenterX, 48, { align: "center" });
  doc.setDrawColor(0, 0, 0);
  doc.setLineWidth(1);
  ...
  const logoData = await loadImageAsDataUrl(options.logoPath || "/Logo.png");
  if (logoData) {
  ...
  drawStyledLogoText(doc, logoX + logoSize / 2, logoY + logoSize / 2 + 4);
...
const ensureSpace = (doc, y, neededHeight) => {
  ...
  if (y + neededHeight < pageHeight - 36) return y;
  doc.addPage();
  return 44;
};
